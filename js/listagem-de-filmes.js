...
const criarCardFilme = (filme) => {
    ...
    divImagem.addEventListener('click', async () => {
        const resposta = await (await fetch(`https://tmdb-proxy.cubos-academy.workers.dev/3/movie/${filme.id}?language=pt-BR`)).json()
...
const init = async () => {
    await pegarListaFilmes("https://tmdb-proxy.cubos-academy.workers.dev/3/discover/movie?language=pt-BR&include_adult=false")
    separarFilmes(0, 5)
